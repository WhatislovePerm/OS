...
3). Вызвал vi и создал файл hello.sh с помощью команды «vi hello.sh» (алгоритм действий представлен на рис. -@fig:001 , -@fig:002 ).
...
![Режим "вставка" для работы с текстом](image/3.png){ #fig:003 width=70% }
...
7). Нажал «w» (записать) и «q» (выйти), далее нажимаю клавишу «enter» для сохранения текста и завершения работы (См. рисунок -@fig:006 ).
...
5). Перешел в режим вставки, нажав клавишу «i», и набрал следующий текст: local. Нажал «esc» для возврата в командный режим (Скриншот -@fig:013 ).
